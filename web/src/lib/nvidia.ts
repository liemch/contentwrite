  ...
  for (const line of raw.split("\n")) {
    ...
    try {
      ...
      if (json.error?.message) {
        throw new Error(`NVIDIA API: ${json.error.message}`);
      }
      text +=
      ...
      if (e instanceof Error && e.message.startsWith("NVIDIA API:")) throw e;
    }
  }
  return text;
}
...
async function chatViaFetchStream(
  ...
  try {
    res = await fetch(`${BASE_URL}/chat/completions`, {
      ...
      signal: AbortSignal.timeout(VERCEL_CHAT_MS),
    });
  } catch (error) {
    const aborted =
  ...
  if (!res.ok) {
    ...
    throw new Error(`NVIDIA HTTP ${res.status}: ${raw.slice(0, 200)}`);
  }
